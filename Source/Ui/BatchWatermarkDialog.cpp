...
public:
    ...
    void run() override {
        ...
        auto callbackFim = onConcluido_;
        juce::MessageManager::callAsync([callbackFim, sucesso, exportados, resultadoFinal, erros]() {
            if (callbackFim) {
                callbackFim(sucesso, exportados, resultadoFinal, erros);
            }
        });
    }
...
private:
    ...
    // próprio SafePointer pro diálogo (ver o construtor de
    // ExportWatermarkThread), então a cópia é auto-suficiente e segura
    // mesmo com o worker e o diálogo mortos.
    void notificarProgresso(double valor, const juce::String& msg) {
        auto callback = onProgresso_;
        juce::MessageManager::callAsync([callback, valor, msg]() {
            if (callback) {
                callback(valor, msg);
            }
        });
